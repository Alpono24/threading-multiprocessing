...
logging.info(f'Вариант 1 - последовательное выполнение/последовательный код: Работа функции download_content.')

for url in urls:
    response = download_content(url)
    # if response:
    #     logging.info(f'Содержимое {url} успешно загружено!')
    # else:
    #     logging.info(f'Ошибка в загрузке {url}')
